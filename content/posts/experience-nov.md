---
title: "experience"
date: 2025-11-11T13:09:04+05:30
unlisted: true
_build:
  list: false
  render: true
...
- **what i did**:
  ...
  - i set up the system to collect events (like user actions or transactions) from sources like dynamodb and sftp.
  - i used tools like kinesis, firehose, and kafka to bring in the data quickly.
  - every day, the system handles over **3 million events**.

- **technical details**:

  - i processed this data using spark on glue jobs with **dbt** for modeling.
...
- **challenges**:
  ...
  - i had to ensure it could scale as the user base grew to **100,000+ daily active users (dau)**.
...
**project 2: automating elt pipelines**
...
- **impact**:
  ...
  - it improved features like subscriptions (recurring payments) and chat-and-pay (ai-based payment tools), making the platform more user-friendly and boosting engagement amid **$1.28 million phase 3 presale surge**.
...
- **technical details**:
  ...
  - i optimized these jobs to cut processing time by 40%, handling tasks like tracking price history, subscription flows, gold investment plans (gip), and finance dashboards with **2x volume** post-app refresh.
...
- **technical details**:
  ...
  - i integrated streams and **cdc** with dynamodb and s3 to manage delinquent payments, dormant accounts, and overall risks.
...
- **what i did**:
  ...
  - i created tableau dashboards for things like kyc funnels, revenue insights, and gold redemption trends.
...
- **what i did**:
  ...
  - i set up ci/cd using git and **jenkins** (with rancher for container orchestration) for deploying pipelines.
...
- technical: building **elt/etl pipelines**, real-time streaming with kafka, data processing with spark, orchestration with airflow, **dbt modeling**, storage in s3/redshift, and visualizations in tableau.
- also: ci/cd with python, git, and jenkins/rancher.
