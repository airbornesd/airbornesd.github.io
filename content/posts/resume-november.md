---
title: "resume november"
date: 2025-11-11T13:07:29+05:30
draft: false
---
...
- **first data hire** at 2 startups; delivered **real-time elt/etl, microservices, dashboards, pipelines**.
...
- built aws data platform from scratch, ingesting **3m+ events/day** with dynamodb/sftp → kinesis/kafka → real-time analytics (<150 ms latency) amid **$1.28m+ phase 3 presale**.
- automated **elt** with airflow/glue + **dbt** for modeling → reduced support tickets 65%, boosted subscription + chat-and-pay for **100k+ dau**.
...
- modernized **etl** stack with python/sql/glue/lambda → 40% faster processing (transactions, payments, kyc) handling **2x volume** post-app refresh.
- implemented kafka/firehose streaming + dynamodb/s3 **cdc** → fraud detection + wallet monitoring.
...
- automated ci/cd with git/**jenkins**/rancher → faster releases, secure payments, subscription renewals.
...
- **technical**: elt/etl pipelines, real-time streaming, cdc, microservices, apis, dynamodb, cockroachdb, s3, segment, sftp, kafka, kinesis, spark, airflow, **dbt**, aws glue, lambda, redshift, athena, tableau, ci/cd (python, git, jenkins, rancher)
